...
Run from this folder, with real internet access:
    ...
    python run_full_population_check.py

Writes run_full_population_check_output.txt next to this script, and (if
the fetch succeeds) a real tracker_full_population.db you can inspect.

NOTE: this file is deliberately NOT named *_test.py / test_*.py — pytest's
default collection glob matches both patterns, and a script with
top-level side-effecting code (network calls, an interactive
input()-before-exit) being auto-imported and executed during `pytest`
collection is exactly the kind of surprise that pattern causes. Keep it
named this way; do not rename it back to something matching *_test.py.
"""
...
_LOG_PATH = Path(__file__).parent / "run_full_population_check_output.txt"
...
coverage_pct = (100 * distinct_players_with_projection / population_size) if population_size else 0.0
print(
    f"\nDistinct players with a valid weekly projection: "
    f"{distinct_players_with_projection} / {population_size} ({coverage_pct:.1f}%)"
)

# Backend v1 contract (2026-09 review): fetch() succeeding no longer implies every
# player has a projection. A missing one (bye week, off-roster that week, etc.) is
# expected absence and is skipped per-player rather than fatal — see
# espn_adapter.py's fetch() and PHASE_4C_STATUS.md for the full rationale and the
# survey evidence this is based on. Partial coverage here is therefore the NORMAL
# outcome, not something to investigate on its own.
print(
    "\n[EXPECTED] partial coverage is normal under the current contract — a missing "
    "projection for some players (bye weeks, players off an active/relevant roster "
    "that week, etc.) is expected absence, skipped per-player, and does not indicate "
    "a problem by itself. [FAIL] or [ANOMALY] further up (if this run had hit them) "
    "would still mean something is genuinely wrong."
)
if distinct_players_with_projection == 0:
    print(
        "\n[SUSPICIOUS] zero players produced ANY projection at all — that's not partial "
        "coverage, it's total absence, and unlike a partial number this is worth investigating "
        "(e.g. wrong season/week requested, or ESPN's response shape has changed)."
    )
