...
PASS, FAIL, or NOT TESTABLE — NOT TESTABLE means "this needs code this
session doesn't have access to (the tracker's DB/ingestion layer, other
source adapters)", not "this was skipped" or "this failed".

A missing ESPN positional ranking on a COMPLETED season is an expected
source limitation (see FINDINGS.md section D / PHASE_4C_STATUS.md) and
is asserted as such below — it must never show up as a FAIL.
...
def record(name: str, status: str, detail: str = "") -> None:
    assert status in ("PASS", "FAIL", "NOT TESTABLE")
...
if not obs_2026_wk1:
    record("QC2: projection schema correctness", "NOT TESTABLE", "QC1 produced no data to check")
elif bad:
...
if not obs_2026_wk1:
    record("QC3: rankings have correct ranking_type", "NOT TESTABLE", "QC1 produced no data to check")
elif bad:
...
if not obs_2026_wk1:
    record("QC4a: no fabricated ACTUAL for unplayed 2026 week", "NOT TESTABLE", "QC1 produced no data to check")
elif check_4a_ok:
...
if check_4b_ok:
    record("QC4b: 2025 positional ranking correctly absent (source limitation, not a bug)", "PASS", detail_4b)
else:
    record("QC4b: 2025 positional ranking correctly absent (source limitation, not a bug)", "FAIL", detail_4b)
...
# which this session has no access to — marked NOT TESTABLE.
# ------------------------------------------------------------------
try:
...
if not all_checked:
    record("QC6a: every observation attributed source='espn'", "NOT TESTABLE", "no data to check")
elif bad_source:
...
n_nt = sum(1 for _, s, _ in results if s == "NOT TESTABLE")
for name, status, detail in results:
    print(f"  [{status:13s}] {name}")
print(f"\n{n_pass} PASS, {n_fail} FAIL, {n_nt} NOT TESTABLE")

if n_fail:
    print("\nRESULT: QC FAILED — see FAIL lines above.")
    _pause_before_exit(1)
else:
    print("\nRESULT: QC PASSED (NOT TESTABLE items require the tracker's DB/ingestion code and other adapters to close out).")
