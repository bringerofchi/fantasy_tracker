"""
Real Week 1, 2026 ESPN weekly pull -- live network call, full player
...
Week 1's ESPN data is actually available (projections should be
available now; actuals will only populate once games are played).

Run from the repo root:
    python run_espn_week1_pull.py
"""
import sys
...
def main():
    ...
    print()
    print("=== Sample of what landed (projections, week 1, source=ESPN) ===")
    rows = conn.execute("""
    ...
    print()
    print("=== Totals landed this run (projections, week 1, source=ESPN) ===")
    total = conn.execute("""
